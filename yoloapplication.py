...
image = cv2.imread(src_path + "maxresdefault.jpg")
orwidth, orheight = image.shape[1], image.shape[0]
...
blob = cv2.dnn.blobFromImage(image, 1 / 255, (YOLO_IMG_SIZE, YOLO_IMG_SIZE), True,
...
show_detected_image(image, predicted_objects, box_loc, class_id, conf_val, orwidth / YOLO_IMG_SIZE,
                    orheight / YOLO_IMG_SIZE)

cv2.imshow('YOLO_ALGO', image)
cv2.waitKey()
